        ...
        for i, (request_packet, response_packet) in enumerate(packet_pairs):
            try:
                ...
                if len(self.timing_measurements) > 0:
                    prev_latency = self.timing_measurements[-1]['latency_us']
                    jitter_us = abs(latency_us - prev_latency)
                else:
                    jitter_us = 0.0
                ...
                if latency_us > 1000:  # > 1ms is critical
                    timing_anomalies.append({
                        'type': 'Fronthaul Critical Latency',
                        'description': f'DU-RU latency {latency_us:.1f}μs exceeds 1000μs threshold',
                        'severity': 'critical',
                        'fronthaul_latency_us': latency_us,
                        'timing_jitter_us': jitter_us,
                        'packet_number': i
                    })
                    self.ecpri_statistics['timing_violations'] += 1
                    
                elif latency_us > 100:  # > 100μs is high
                    timing_anomalies.append({
                        'type': 'Fronthaul High Latency',
                        'description': f'DU-RU latency {latency_us:.1f}μs exceeds 100μs threshold',
                        'severity': 'high',
                        'fronthaul_latency_us': latency_us,
                        'timing_jitter_us': jitter_us,
                        'packet_number': i
                ...
                if jitter_us > 50:  # > 50μs jitter
                    timing_anomalies.append({
                        'type': 'Fronthaul Timing Jitter',
                        'description': f'Timing jitter {jitter_us:.1f}μs indicates unstable fronthaul',
                        'severity': 'medium',
                        'fronthaul_latency_us': latency_us,
                        'timing_jitter_us': jitter_us,
                        'packet_number': i
                ...
                print(f"Timing measurement error for packet pair {i}: {e}")
                continue
        
        return timing_anomalies
    
    def monitor_fronthaul_bandwidth(self, packets, time_window=1.0):
        """Monitor fronthaul bandwidth utilization and detect anomalies"""
        bandwidth_anomalies = []
        packet_sizes = []
        timestamps = []
        
        print("Monitoring fronthaul bandwidth utilization...")
        
        for i, packet in enumerate(packets):
            try:
                packet_size = len(packet) if hasattr(packet, '__len__') else getattr(packet, 'len', 64)
                packet_time = getattr(packet, 'time', time.time() + i * 0.001)
                
                packet_sizes.append(packet_size)
                timestamps.append(packet_time)
                
                # Analyze bandwidth in sliding windows
                if len(timestamps) > 100:  # Analyze every 100 packets
